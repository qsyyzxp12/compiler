...
%type <node> add_op mul_op dim_list type_decl nonempty_assign_expr_list else_if_list
...
param		: type ID 
			...
			makeFamily($$, 2, $1, makeIDNode($2, NORMAL_ID));
                ...
                {
			$$ = makeDeclNode(FUNCTION_PARAMETER_DECL);
			AST_NODE* array_id = makeIDNode($2, ARRAY_ID);
			makeChild(array_id, $3);
			makeFamily($$, 2, $1, array_id);
                };
...
block           : decl_list stmt_list 
		{
			$$ = Allocate(BLOCK_NODE);
			AST_NODE* decl_list = makeChild(Allocate(VARIABLE_DECL_LIST_NODE), $1);
...
id_list		: ID 
		...
		| id_list MK_COMMA ID 
                {
			$$ = $1;
			makeSibling($$, makeIDNode($3, NORMAL_ID));
                }
